...
library(vegan)
Ds <- diversity(data[-1], index = "simpson")
Ds
#OR
simp <- diversity(dat_a, index = "simpson", MARGIN = 1) # data_a "data" without the sample column
simp

## using vegan for Shannon's Index (H'):
library(vegan)
H <- diversity(data[-1], index = "shannon")
H
#OR
sha <- diversity(data, index = "shannon", MARGIN = 1) # data "flut" without the sampple row
sha
...
# Pielou
J <- H/log(specnumber(dat_a))# data_a "data" without the sample column
J
