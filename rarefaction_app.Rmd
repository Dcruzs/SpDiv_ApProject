...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # change sample size for rarify plot 
      #  sliderInput("rarfy_sampl", label = "Sample Size of the Rarefied Plot", 
      #            min = 1, max = 50, value = 10),
      
      conditionalPanel(
        condition = "input.dataset == 'dune_vg'",
        sliderInput("rarfy_sampl_dune", label = "Change Sample Size:", 
                    min = 1, max = 50, value = 15)
      ),
      conditionalPanel(
        condition = "input.dataset == 'mite_vg'",
        sliderInput("rarfy_sampl_mite", label = "Change Sample Size:", 
                    min = 1, max = 300, value = 8)
      ),
      conditionalPanel(
        condition = "input.dataset == 'BCI_vg'",
        sliderInput("rarfy_sampl_BCI", label = "Change Sample Size:", 
    ...
    mainPanel(
      ...
      plotlyOutput("plot_rarefy", width = "650px"),
...
server <- function(input, output) {
  ...
  my_acc_curve <- reactive({
    dataset <- switch(input$dataset,
                      "dune_vg" = dune,
                      "mite_vg" = mite,
                      "BCI_vg" = BCI)
    
    acc <- switch (input$dataset,
                   "dune_vg" = specaccum(dune_vg[, -(1:6)]),
                   "mite_vg" = specaccum(mite_vg[ ,-(1:6)]),
                   "BCI_vg" = specaccum(BCI_vg[ ,-(1:10)])
    )
                   
                   
    acc_df <- data.frame(sites = acc$sites, richness = acc$richness, sd = acc$sd)
    
    
    # Define ylim based on the selected dataset
    if (input$dataset == "dune_vg") {
      ylim <- c(0, 35)
    } else if (input$dataset == "mite_vg") {
      ylim <- c(0, 40)
    ...
    plot_acc <- ggplot(data = acc_df, aes(x = sites, y = richness)) +
      geom_line(colour = "black", size = 1) +
      geom_ribbon(aes(ymin = richness - sd, ymax = richness + sd), 
                  fill = "#2b8cbe", alpha = 0.2) +
      ylim(ylim) +  # Use the defined ylim 
